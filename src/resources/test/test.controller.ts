...
export class TestController {
  constructor(private readonly ttsService: TextToSpeechService, private readonly fileStorageService: GoogleCloudStorageService) { }
  @Post()
  async handleTextToSpeech(@Body() dto: textToSpeechDto, @Res() res: Response) {
    const audioContent = await this.ttsService.convertTextToSpeech(dto.text, dto.voice);
    const url = await this.fileStorageService.uploadBuffer(audioContent, "records");
    res.json(url);
  }
